...
class DaemonClient:
    ...
    def get_mappings(self) -> List[MidiMapping]:
        ...
        try:
            mappings_json = self.proxy.GetMappings(
    ...
    def add_mapping(self, mapping: MidiMapping) -> bool:
        ...
        try:
            ...
            return bool(self.proxy.AddMapping(
                mapping_json,
                dbus_interface=DBUS_INTERFACE_NAME
            ))
        except Exception as e:
    ...
    def remove_mapping(self, index: int) -> bool:
        ...
        try:
            return bool(self.proxy.RemoveMapping(
                index,
                dbus_interface=DBUS_INTERFACE_NAME
            ))
        except Exception as e:
    ...
    def get_midi_devices(self) -> List[MidiDevice]:
        ...
        try:
            devices_json = self.proxy.GetMidiDevices(
                dbus_interface=DBUS_INTERFACE_NAME
            )
    ...
    def get_pipewire_nodes(self) -> List[PipewireNode]:
        ...
        try:
            nodes_json = self.proxy.GetPipewireNodes(
                dbus_interface=DBUS_INTERFACE_NAME
            )
